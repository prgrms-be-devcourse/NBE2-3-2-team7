...
<template>
	<main class="flex-grow flex flex-col items-center">
		<section class="flex mt-4 w-full justify-center">
			<form id="edit-place-form" class="max-w-4xl px-4 flex flex-col w-full bg-white"
				enctype="multipart/form-data">
				<div class="flex justify-between pb-2 border-b border-gray-300">
					<router-link to="/user/land" class="font-bold p-2">&#10094;&#10094; 목록</router-link>
					<button onclick="deleteRentalPlace()"
						class="p-2 text-red-500 rounded-lg font-bold text-sm hover:text-red-700 transition-colors">임대지
						삭제</button>
				</div>
						...
						<div class="bg-white px-4 py-2 flex-1 border border-gray-300 rounded-md">
							...
							<div class="flex items-center space-x-2">
								<input id="price" type="text" placeholder="금액을 입력해주시기 바랍니다."
									class="w-full h-9 p-1 focus:outline-[#3FB8AF]">
				...
				<div class="flex flex-col mt-4 border-gray-300">
					...
					<div class="flex justify-between border-t pt-2 border-gray-300">
						...
						<button type="submit"
							class="py-2 px-4 bg-[#3FB8AF] flex-grow-0 text-white font-bold rounded-lg transition-colors hover:bg-[#2c817c]">수정하기</button>
					</div>
				</div>
			</form>
		</section>
