				...
				<form id="edit-user-form" class="min-w-5xl space-y-12 p-8 flex-col" enctype="multipart/form-data"
					...
					<div class="flex sm:flex-row flex-col justify-between sm:space-y-0 space-y-8">
						<div class="space-y-4 flex flex-col justify-center items-center">
							<img id="profile-image" :src="image"
							...
							<input type="file" id="profile-real-upload" accept="image/gif, image/png, image/jpeg"
								class="hidden">
							<button id="profile-custom-upload" onclick="uploadImage()"
						...
						<div class="flex space-x-4 justify-center">
							<div class="flex flex-col space-y-4">
								...
								<span v-if="role === 'landlord'"
									class="h-9 block p-1 font-bold text-gray-700 flex-shrink-0">사업자등록번호</span>
								<span class="h-9 block p-1 font-bold text-gray-700 flex-shrink-0"> {{ role ===
									'landlord' ? '상호명' : '닉네임' }}</span>
							</div>
							<div class="flex flex-col space-y-4">
								<span class="h-9 p-1 font-bold text-gray-700" id="user-email">{{ email }}</span>
								<input type="password" id="password"
									class="border p-1 h-9 border-gray-300 focus:outline-[#3FB8AF]"
									placeholder="변경할 비밀번호 입력" autocomplete="new-password">
								...
								<span v-if="role === 'landlord'" class="h-9 p-1 font-bold text-gray-700"
									id="landlord-num">{{ businessId }}</span>
								<input type="text" id="brand"
									class="border p-1 h-9 border-gray-300 focus:outline-[#3FB8AF]"
									:placeholder="role === 'landlord' ? '상호명 입력' : '닉네임 입력'" />
							</div>
						</div>
					</div>
					<div class="flex justify-between w-full">
						<button id="delete-account-btn" type="button"
							class="text-red-500 font-bold p-2 hover:text-red-700 transition-colors rounded-md hover:bg-red-50">회원
							탈퇴</button>
						<!-- 추후 수정 ) 수정 내용 유무에 따라 데이터 전달할 것 -->
						<button type="submit"
							class="text-white bg-[#3FB8AF] hover:bg-[#2c817c] p-2 rounded-md transition-colors font-bold"
							@click="goBack">정보 수정</button>
					</div>
				</form>
			</div>
		</section>
